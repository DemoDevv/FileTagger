...
def test_analyze_document_with_incompatible_extension():
    ...
    assert (
        local_document_analyzer._extract_text(file_path, file_path.suffix.lower())
        is None
    )
...
def test_analyze_document_title_case_insensitive():
    ...
    tags = local_document_analyzer.analyze_document(file_path)

    assert tags is not None and tags["tags"] == "python"
